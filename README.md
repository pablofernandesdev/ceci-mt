<h1 align="center"> CeciMT </h1>

> Status do Projeto: Concluído :heavy_check_mark:

## Descrição do Projeto
<p align="justify"> O projeto CeciMT é um projeto que possui arquitetura e funcionalidade básicas para o início de novos projetos multitenant. 

Confira também o projeto [CeciAdminMT](https://github.com/pablofsilva91/ceci-admin-mt), que contém arquitetura e funcionalidade básicas para o início de novos projetos cujo objetivo seja administrar aplicações multitenant, ou seja, realizar a gestão como manter e adicionar novos tenants para a aplicação, ações que não estão disponíveis no projeto CeciMT. 
...
- Tenants
    - Controle de leitura e gravação de acordo com a chave da empresa
...
:dart: Controle de leitura e gravação do tenant de acordo com a chave de acesso da empresa. 
...
```
git clone https://github.com/pablofsilva91/ceci-mt.git
```
...
1. Criando tabelas com a migração inicial do projeto
    ...
    ```
    dotnet ef database update --project CeciMT.Infra.Data --startup-project CeciMT.WebApplication
    ```
...
2. Realizar a migração do zero
    ...
    Para criar a migração inicial do zero, é necessário excluir a pasta "Migrations" que consta no projeto "CeciMT.Infra.Data". A nova migração pode ser criada com o comando "dotnet ef migrations add [NOME_MIGRACAO]". Por conta da estrutura do projeto é necessário especificar em qual projeto está a classe de contexto e qual é o projeto de inicialização:

    ```
    dotnet ef migrations add InitialMigration --project CeciMT.Infra.Data --startup-project CeciMT.WebApplication
    ```
...
```
dotnet ef migrations add [NOME_MIGRACAO] --project CeciMT.Infra.Data --startup-project CeciMT.WebApplication
```
...
Navegue até o projeto WebApplication "CeciMT.WebApplication" e execute o comando "dotnet run": 

```
PS ...\CeciMT\CeciMT.WebApplication> dotnet run
```
...
Navegue até o projeto de testes "CeciMT.Test" e execute o comando "dotnet test": 

```
PS ...\CeciMT\CeciMT.Test> dotnet test
```
...
O arquivo "DockerFile", já está contemplado nessa versão e está disponível na raiz da solução, em alguns casos o arquivo é armazenado na raiz do projeto WebApplication, mais como o projeto CeciMT possui mais de um projeto na solução e são dependências de projetos, a publicação no Heroku só foi concluída com essa organização. Vale ressaltar que essa foi uma configuração na qual foi possível publicar o projeto CeciMT no Heroku, não necessáriamente é uma regra para o caminho do arquivo "DockerFile".
